...
- `configs/`、`docker-compose.yml`、`.env.example`：语料路径、Engine 版本、CodeRAG store、MCP endpoint、provider、reranker、检索 top-k 等会改变召回行为的配置。
- `eval/datasets/`、`src/codalith/eval/`、`tests/test_*eval*.py`：评估数据、指标、runner、判定口径。

最小本地验证命令：

```bash
uv run pytest tests/test_cards_eval.py tests/test_reranker_and_mcp_eval.py tests/test_ue57_common_issue_benchmark.py
