...
| 时间 | 动作 | 说明 |
| --- | --- | --- |
| 2026-07-09 | UE 语料/验收分离 | 完整 UE 5.7.4 语料迁至 `configs/ue_*` + `CODALITH_UE_*`；默认 `mcp-http` 保持 sample；`mcp-http-ue`（profile `ue`）挂完整 embedding store；UE Eval Suite 仅 `eval/datasets/` + profile `eval-ue` 显式加载。 |
...
| 2026-07-08 | 去 UE 定向化 | Codalith 默认定位收口为通用源码语料 MCP 网关：默认 registry / compose / tests 使用 `sample-codebase`，核心源码不再包含 UE extractor、旧 UE schema migration 或 UE 默认路径。 |
...
默认 MCP 使用 sample corpus。完整 UE 5.7.4 语料是可选产品语料（`configs/ue_5_7_4_registry.json` + 本地 embedding store），通过本地 `.env` 切换或 `docker compose --profile ue up -d mcp-http-ue` 启动。UE Eval Suite 只是验收数据集（`eval/datasets/ue_eval_suite.jsonl`），由 profile `eval-ue` / `scripts/ue-mcp-eval.sh` 显式加载。
...
9. **评估层**（`codalith.eval`）：指标、共享 `evaluate_dataset` 核心与 in-process / MCP 两个 runner；默认 sample dataset，UE Eval Suite 需显式传 UE corpus registry + `eval/datasets/ue_eval_suite.jsonl`。
...
| configs | `configs/` | sample 与可选 UE corpus 配置（registry / priors / seed cards） |
...
UE Eval Suite 显式命令（复用产品 UE registry，数据集在 `eval/datasets/`）：
...
- 不要把 UE 相关代码放回 `src/codalith` 默认核心路径；完整 UE 语料通过 `configs/ue_*` 显式配置，验收只加载 `eval/datasets/ue_eval_suite.jsonl`。
- CodeRAG 子模块不要做无关修改。
