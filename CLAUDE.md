...
| 时间 | 动作 | 说明 |
| --- | --- | --- |
| 2026-07-08 | 去 UE 定向化 | Codalith 默认定位收口为通用源码语料 MCP 网关：默认 registry / compose / docs / tests 使用 `sample-codebase`，核心源码不再包含 UE extractor、旧 UE schema migration 或 UE 默认路径。UE 5.7 仅保留为 `eval/` 专用 registry、source priors、seed cards 和 dataset，因为当前 CodeRAG embedding 质量基线来自 UE 5.7 源码。 |
...
2. **语料层**（`codalith.corpus`）：`CorpusRegistry`、`URIResolver`、`SourcePolicy`、`SourceReader`。
3. **检索适配层**（`codalith.coderag`）：`CodeRAGAdapter`，支持 native CodeRAG 与本地确定性 fallback。
4. **语义层**（`codalith.semantic`）：SQLite/Postgres 语义图、通用 semantic input types；默认无 extractor profile 时成功 no-op。
5. **编译层**（`codalith.compiler`）：意图识别、实体检测、corpus-local source priors、检索规划、重排、证据选择，输出 Context Pack。
6. **卡片层**（`codalith.cards`）：seed card schema、生成、哈希、Markdown 渲染、验证。
7. **网关层**（`codalith.gateway`）：MCP stdio + Streamable HTTP、工具/资源/审计/鉴权。
8. **任务层**（`jobs/`）：CLI 入口。
9. **评估层**（`codalith.eval`）：指标和 runner；默认 sample dataset，UE eval 需显式传 eval registry/dataset。
...
-> plan_queries -> CodeRAGAdapter.search_code
...
| coderag | `src/codalith/coderag/` | CodeRAG native/local adapter、查询结果映射 |
| compiler | `src/codalith/compiler/` | Context Pack 编译 |
| semantic | `src/codalith/semantic/` | 语义图 store、通用语义类型、profile runner |
...
通过标准：UE Eval Suite `count == 80`，`file_recall@5 == 1.000`，`candidate_file_recall == 1.000`，`module_accuracy == 1.000`，HTTP MCP eval 的 `failure_class` 全部为 `pass`。
