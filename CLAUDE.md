...
2. **语料层**（`codalith.corpus`）：`CorpusRegistry`（配置键 `corpora`/`projects`/`generated`，`get_base` 解析默认语料）、`URIResolver`、`uris` URI 原语（含 `parse_source_uri`）、`SourcePolicy`、`SourceReader`（读失败抛 `SourceReadError`）。
...
4. **语义层**（`codalith.semantic`）：SQLite/Postgres 语义存储，通用模型为 modules / module_deps / symbols / compile_guards / knowledge_cards / graph_edges；core 不含任何领域 extractor，`semantic_profile` 非空即配置错误。
...
6. **卡片层**（`codalith.cards`）：seed card schema、生成、哈希、Markdown 渲染、验证、`is_card_path` 判定。
