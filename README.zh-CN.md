...
Codalith 是一个面向版本化源码语料的 Python MCP 网关。它在 CodeRAG 风格检索之上增加语料解析、源码读取策略、审计日志、语义抽取、Knowledge Card 校验和评测工具。对 MCP 客户端的能力声明完全由 corpus 注册表配置驱动，而非硬编码；当前部署内置 Unreal 域包并服务于 Unreal Engine 5 源码。

## 功能

- 面向 AI 编程工具的 MCP stdio 和 Streamable HTTP 网关。
- 可配置源码路径、索引路径、CodeRAG 存储路径和卡片路径的版本化语料注册表。
- 统一 `codalith://<corpus_id>/...` 资源的源码 URI 解析。
- 源码读取策略、scope 和审计日志。
- 可插拔语义抽取 profile；内置 `unreal` profile 覆盖 Build.cs 依赖、UHT 反射、C++ 符号和编译宏条件。
...
- 可选：待索引语料的本地或远程源码 checkout（当前部署使用 Unreal Engine）。
...
运行可选真实语料 smoke path（当前部署挂载 UE 源码树）：
