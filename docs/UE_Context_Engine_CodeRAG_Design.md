# UE Context Engine powered by CodeRAG 项目设计文档
...
**UE Context Engine powered by CodeRAG** 是一个面向 AI Coding Agent 的 UE 源码上下文服务。
...
> CodeRAG 负责“快速找到相关代码”；UE Context Engine 负责“判断这些代码在 UE 的版本、模块、反射、构建、平台、项目语境中到底意味着什么”；MCP Gateway 负责“把这个证据包安全地交给 Claude Code、Codex、Cursor”。
...
```text
CodeRAG = Retrieval Core
UE Context Engine = UE-aware Context Layer
```
...
engines:
  ue-5.7.4:
    ...
    indexed_root: "/srv/ue-context/corpora/ue-5.7.4"
    coderag_store: "/var/lib/ue-context/coderag/ue-5.7.4"
    semantic_schema: "ue_5_7_4"
    card_root: "/srv/ue-context/cards/ue-5.7.4"
    default: true
    access_scopes:
      - "ue:5.7"
      - "source:read"
  ...
  ue-5.8.0:
    ...
    indexed_root: "/srv/ue-context/corpora/ue-5.8.0"
    coderag_store: "/var/lib/ue-context/coderag/ue-5.8.0"
    semantic_schema: "ue_5_8_0"
    card_root: "/srv/ue-context/cards/ue-5.8.0"
    access_scopes:
      - "ue:5.8"
      - "source:read"
...
projects:
  ProjectA:
    ...
    indexed_root: "/srv/ue-context/corpora/project-a"
    coderag_store: "/var/lib/ue-context/coderag/project-a"
    semantic_schema: "project_a"
    card_root: "/srv/ue-context/cards/project-a"
    access_scopes:
      - "project:ProjectA"
      - "source:read"
```
...
```text
/srv/ue-context/corpora/ue-5.7.4/
...
- CodeRAG get_file 必须经过 ue_read_source 的权限、行数、审计包装。
...
```text
ue_lookup_symbol("AActor")
ue_lookup_symbol("TArray")
ue_lookup_symbol("GetLifetimeReplicatedProps")
```
...
`ue_context` 返回结构化 Context Pack。MCP tool result 应尽量使用 structured content，同时保留短文本摘要以兼容不同 host。
...
```json
{
  ...
  "summary": {
    ...
    "generated_by": "ue-context-engine"
  },
...
Call ue_context before answering questions about engine implementation, API behavior, modules, UHT, reflection, Build.cs, networking, rendering, gameplay framework, editor internals, assets, GC, serialization, or version-specific behavior.
...
源码文件通过 resource template 或 `ue_read_source` 读取。
...
```text
/srv/ue-context/corpora/project-a/
  Source/
  Plugins/
  Config/
  ProjectA.uproject
...
CREATE INDEX idx_ue_graph_from
ON ue_graph_edges(corpus_id, from_node, edge_type);

CREATE INDEX idx_ue_graph_to
...
```bash
export CODERAG_WATCHED_DIR=/srv/ue-context/corpora/ue-5.7.4
export CODERAG_STORE_DIR=/var/lib/ue-context/coderag/ue-5.7.4
...
```bash
claude mcp add --transport http ue-context https://mcp.company.internal/ue/mcp \
...
1. Call `ue_context` first.
2. Use UE 5.7.4 unless the user asks for another version.
3. Do not rely on generic memory when source-backed UE context is available.
...
### 27.2 UE Context v0 验收

```text
- ue_context 能返回 Context Pack。
- Context Pack 包含 version、modules、symbols、source_spans、cards、caveats。
- ue_read_source 强制行数限制。
- 所有 source read 有 audit log。
- 未授权 corpus 访问被拒绝。
- AI 客户端里 UE 问题会优先看到 ue_context。
```
...
2. 工具名使用 ue_context，不使用 generic search。
3. 项目 CLAUDE.md / AGENTS.md 写 grounding policy。
4. 只把 ue_context 作为核心入口。
5. 其他工具延迟暴露。
```
...
```text
ue_context 返回符合 schema 的 Context Pack。
...
src/ue_context/corpus/registry.py
src/ue_context/corpus/uri_resolver.py
src/ue_context/corpus/source_policy.py
src/ue_context/coderag/adapter.py
src/ue_context/compiler/context_pack.py
src/ue_context/compiler/context_compiler.py
src/ue_context/gateway/mcp_server.py
src/ue_context/gateway/tools.py
src/ue_context/semantic/extractors/build_cs.py
src/ue_context/semantic/extractors/uht_reflection.py
src/ue_context/cards/schema.py
src/ue_context/cards/verifier.py
eval/datasets/ue50.jsonl
```
...
2. MCP Gateway 暴露 ue_context / ue_read_source / ue_index_status。
3. ue_context 内部调用 CodeRAG，返回 Context Pack v0。
4. ue_read_source 可以按 ue:// URI 读取受限源码范围。
