...
from ue_context.cards.generator import built_in_cards
...
def main(argv: list[str] | None = None) -> int:
    ...
    verifier = KnowledgeCardVerifier(URIResolver(registry), CodeRAGAdapter(registry))
    corpus = registry.get_engine(args.version)
    results = [
        {"card_id": card.card_id, **verifier.verify(card).as_dict()}
        for card in built_in_cards(corpus_id=corpus.corpus_id, version=corpus.ue_version or args.version)
    ]
